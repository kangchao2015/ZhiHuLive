    ...
    def step7(self):
        ...
        while item_count_done < total_count:
            ...
            if is_first_curl:
                ...
                target_api = "https://api.zhihu.com/lives/%d/messages?after_id=%d&chronology=asc&limit=30" % (
                self.live_id, last_item_id);
            ...
            if r.status_code == 200:
                ...
                for k, v in data_obj.items():
                    if k == "data":
                            ...
                            if item.has_key('type'):
                                ...
                                if ty == "text":
                                    ...
                                    if item.has_key('audio') and item['audio'].has_key('url'):
                                        ...
                                        if not os.path.exists(audio_full_name):
                                            if download(audio_url, local_dir, audio_name) == True:
                                                L.info("[+++][audio] %d download successfully count :%d" % (
                                                item_id, item_count_done));
                                            else:
                                    ...
                                    if self.audio_count == 1:
                                        ...
                                        audio_export(self.audio, self.target_dir, "%s_%s_%d_%d" % (
                                        self.title, self.author, self.audio_file, self.audio_count), "ogg")
                                        self.audio_file += 1;
                                        self.audio_count = 0;
                                    else:
                                        self.audio = audio_append(self.audio, audio_full_name);
                                    ...
                                    if item.has_key('image') and item['image'].has_key('full'):
                                        ...
                                        if not os.path.exists(os.path.join(local_dir, image_name)):
                                            if download(image_url, local_dir, image_name) == True:
                                                L.info("[+++][image] %d download successfully count :%d" % (
                                                item_id, item_count_done));
                                            else:
                                        ...
                                        for pic in item["multiimage"]:
                                            ...
                                            if not os.path.exists(os.path.join(local_dir, pic_name)):
                                                if download(pic_url, local_dir, pic_name) == True:
                                                    L.info("[+++][mpic] %d download successfully count :%d--%d" % (
                                                    item_id, item_count_done, pos));
                                                else:
                                        ...
                                        for vd in item["video"]["playlist"]:
                                            ...
                                            if not os.path.exists(os.path.join(local_dir, video_name)):
                                                if download(video_url, local_dir, video_name) == True:
                                                    L.info("[+++][video] %d download successfully count :%d--%d" % (
                                                    item_id, item_count_done, pos));
                                                else:
                                    ...
                                    if item.has_key('file') and item['file'].has_key('url'):
                                        ...
                                        if not os.path.exists(os.path.join(local_dir, file_name)):
                                            if download(file_url, local_dir, file_name) == True:
                                                L.info("[+++][file] %d download successfully count :%d" % (
                                                item_id, item_count_done));
                                            else:
                        ...
                        # print "%s ==> %s" %(k,v);
                        pass;
                L.info("本次总共抓取条数：%-2d 总计抓取条数：%-4d 当前抓取之前的剩余条数(API)：%-4d 剩余抓取:%d" % (
                count, item_count_done, unload_count, total_count - item_count_done));
            else:
                L.error("目标抓取失败！live id :%d" % self.live_id);
            L.warning("抓取《%s》抓取%d/%d ".encode("utf-8") % (self.title, item_count_done, total_count));
        audio_export(self.audio, self.target_dir,
                     "%s_%s_%d_%d" % (self.title, self.author, self.audio_file, self.audio_count), "ogg")


# a.go();

b = [912339158498959360,854004941730246656,919515786354237440];
for z in b:
