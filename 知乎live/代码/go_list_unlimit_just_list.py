...
def dealliveinfo(live_id):
	...
	try:
		...
		if len(ret) > 0:
			try:
				sql1 = "UPDATE live_infos set accessable = 2 WHERE id = %d " % live_id;
				curson2.execute(sql1);
				
				sql3 = "select updated_at from live_infos where id = %d" % live_id;
				curson2.execute(sql3);
				res = curson2.fetchall()[0];
				if res == (0,):

					#sql2 = "UPDATE live_infos set updated_at = %d WHERE id = %d " % (int(time.time()), live_id);
					sql2 = "UPDATE live_infos set updated_at = %d WHERE id = %d " % (int(time.time()), live_id);
					curson2.execute(sql2);

					with open("../unlimited.txt", "a") as f:
						f.write("%d"  % live_id);

				L.error("%d 已经存在 已经修改为 2!" % live_id);
			except Exception as e:
				print e;
				L.error("%d 记录在数据库中存在 但是 修改失败！" % live_id);
			conn.commit();
			return ;
		else:
			pass;
	...
	sql = sql + ",`accessable`, `updated_at`) VALUES (null"
	...
	for k,v in total_size.items():
		...
		while True:
			...
			for item in target_obj['data']:
				...
				# if live_id in aaaa:
				# 	print "%d 已经有了..." % live_id;
				# else:
				# 	try:
				# 		glive.doit(live_id,1);
				# 	except Exception as e:
				# 		with open("aaaaa.txt","a") as f:
				# 			f.write("%d 抓取失败！" % live_id);
